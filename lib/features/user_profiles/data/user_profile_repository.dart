...
class UserProfileRepository {
  ...
  Future<List<UserProfileAssignment>> fetchProfilesForUser(String userId) async {
    final response = await _client
        .from('user_profiles_view')
        .select(
          'id,user_id,profile_slug,profile_name,assigned_by,created_at',
        )
        .eq('user_id', userId)
        .order('created_at');
  ...
  Future<void> adminSetUserProfiles({
    ...
    required List<String> profileSlugs,
  }) async {
    await _client.rpc(
